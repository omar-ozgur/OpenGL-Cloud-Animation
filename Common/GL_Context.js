...
{	this.string_map = { };	this.m_text = new text_line( 20 ); 		this.start_index = 0;	this.tick = 0; 	this.visible = true;
	...
	Debug_Screen.prototype.display = function(time)
	{
		...
		var begin = model_transform;

		model_transform = mult( model_transform, translation( 0, -3, -23 ) );	
		this.m_text.set_string( "Press 'b' to change" );
		this.m_text.draw( this.graphicsState, model_transform, true, vec4(0,0,0,1) );
		model_transform = mult( model_transform, translation( 0, -1, 0 ) );	
		this.m_text.set_string( "  the time of day" );
		this.m_text.draw( this.graphicsState, model_transform, true, vec4(0,0,0,1) );

		model_transform = begin;
		
		model_transform = mult( model_transform, translation( 0, 10, -14 ) );
		if(animTime/1000 > 3.5 && animTime/1000 < 6){
			this.m_text.set_string( "What a nice day!" );
		}
		else if(animTime/1000 > 7 && animTime/1000 < 10){
			this.m_text.set_string( "I think I'll just" );
			this.m_text.draw( this.graphicsState, model_transform, true, vec4(0,0,0,1) );	
			model_transform = mult( model_transform, translation( 0, -1, 0 ) );	
			this.m_text.set_string( "stare at these" );
			this.m_text.draw( this.graphicsState, model_transform, true, vec4(0,0,0,1) );	
			model_transform = mult( model_transform, translation( 0, -1, 0 ) );	
			this.m_text.set_string( "     clouds" );
		}
		else if(animTime/1000 > 15 && endTime == 0){
			model_transform = mult( model_transform, translation( 0, 10, 1 ) );	
			this.m_text.set_string( "Press Numbers 1-9 to" );
			this.m_text.draw( this.graphicsState, model_transform, true, vec4(0,0,0,1) );	
			model_transform = mult( model_transform, translation( 0, -1, 0 ) );	
			this.m_text.set_string( " change the shape" );
			this.m_text.draw( this.graphicsState, model_transform, true, vec4(0,0,0,1) );	
			model_transform = begin;
			model_transform = mult( model_transform, translation( 0, 10, 0 ) );	
			this.m_text.set_string( "Press 'e'" );
			this.m_text.draw( this.graphicsState, model_transform, true, vec4(0,0,0,1) );	
			model_transform = mult( model_transform, translation( 0, -1, 0 ) );	
			this.m_text.set_string( "when done" );
		}
		else if(animTime/1000 > endTime + 5 && animTime/1000 < endTime + 8 && endTime > 0){
			this.m_text.set_string( "I wish I was a cloud" );
		}
		else{
			this.m_text.set_string( "" );
		}
		this.m_text.draw( this.graphicsState, model_transform, true, vec4(0,0,0,1) );		// Comment this out to not display any strings on the UI
